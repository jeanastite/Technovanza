...
def detect_boxes():
    ...
    smoothed_tvec = np.zeros(3)
    smoothed_rvec = np.zeros(3)
            ...
            for i, corner in enumerate(corners):
                ...
                if rvec is not None and tvec is not None:
                    if calibrated:
                        ...
                        smoothed_tvec = alpha * tvec[0][0] + (1 - alpha) * smoothed_tvec
                        smoothed_rvec = alpha * rvec[0] + (1 - alpha) * smoothed_rvec

                        # Compute relative position
                        relative_tvec = smoothed_tvec - initial_tvec
                        x, y, z = relative_tvec

                        # Compute angular deviation (degree of rotation from the initial orientation)
                        relative_rvec = smoothed_rvec - initial_rvec
                        ...
                        cv2.drawFrameAxes(frame, camera_matrix, dist_coeffs, smoothed_rvec, smoothed_tvec, 0.1)
